...
class SevenZipCompressor:
    ...
    def save_settings(self):
        ...
        self.settings["auto_delete_after_upload"] = self.auto_delete_after_upload_var.get()
    ...
    def refresh_compress_records(self):
        ...
        for i, row in enumerate(cursor.fetchall()):
            # 确保记录有5个字段
            if len(row) == 5:
                self.records_tree.insert("", tk.END, values=(i+1, row[1], row[2], row[3], row[4]))
            else:
                # 跳过格式不正确的记录
                continue
    ...
    def create_compress_tab(self):
        ...
        self.folder_entry = ttk.Entry(frame_folder, textvariable=self.folder_path_var, state="readonly")
        self.folder_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=10, pady=10)
        
        self.browse_btn = ttk.Button(frame_folder, text="浏览", command=self.browse_folder)
        ...
        self.password_var.trace_add("write", lambda *args: self.current_password_var.set(self.password_var.get()))
        ...
        self.use_promotion_var = tk.BooleanVar(value=self.settings.get("use_promotion", True))
        # 添加变化监听，自动保存
        self.use_promotion_var.trace_add("write", self.on_settings_changed)
        promotion_check = ttk.Checkbutton(row1_frame, text="使用宣传文件", variable=self.use_promotion_var, command=self.on_promotion_use_changed)
        ...
        self.promotion_combobox = ttk.Combobox(row1_frame, textvariable=self.selected_promotion_var, values=promotion_names, state="readonly", postcommand=self.update_promotion_list)
        # 绑定选择事件
        ...
        # 自动清理宣传文件选项
        self.auto_clean_promotion_var = tk.BooleanVar(value=self.settings.get("auto_clean_promotion", True))
        self.auto_clean_promotion_var.trace_add("write", self.on_settings_changed)
        auto_clean_check = ttk.Checkbutton(row2_frame, text="压缩完成后自动清理宣传文件", variable=self.auto_clean_promotion_var)
        auto_clean_check.pack(side=tk.LEFT, padx=5)
        
        # 手动清理宣传文件按钮
        ttk.Button(row2_frame, text="手动清理宣传文件", command=self.manual_clean_promotion_files).pack(side=tk.RIGHT, padx=5)
        
        # 自动上传选项
        self.auto_upload_var = tk.BooleanVar(value=self.settings.get("auto_upload", True))
        # 添加变化监听，自动保存
        self.auto_upload_var.trace_add("write", self.on_settings_changed)
        auto_upload_check = ttk.Checkbutton(frame_settings, text="压缩完成后自动上传到百度网盘", variable=self.auto_upload_var)
        auto_upload_check.pack(side=tk.LEFT, padx=5)
        
        # 压缩按钮
        self.compress_btn = ttk.Button(frame_settings, text="开始压缩", command=self.start_compress)
        ...
        # 文本输出
        self.text_output = scrolledtext.ScrolledText(frame_text, wrap=tk.WORD, height=10)
        self.text_output.pack(fill=tk.BOTH, expand=True, padx=10, pady=(10, 0))
        
        # 复制按钮
        button_frame = ttk.Frame(frame_text)
        button_frame.pack(fill=tk.X, padx=10, pady=(5, 10))
        self.copy_btn = ttk.Button(button_frame, text="复制文本", command=self.copy_text)
    ...
    def create_upload_tab(self):
        ...
        generated_title_entry = ttk.Entry(generated_frame, textvariable=self.generated_title_var, state="readonly")
        generated_title_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        ttk.Button(generated_frame, text="复制标题", command=lambda: self.copy_to_clipboard(self.generated_title_var.get())).pack(side=tk.RIGHT, padx=5)
        ...
        ttk.Entry(auto_dir_frame, textvariable=self.auto_dir_var, state="readonly").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        ...
        ttk.Entry(custom_dir_frame, textvariable=self.custom_dir_var).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        ...
        self.custom_extract_code_var = tk.StringVar(value=self.settings.get("custom_extract_code", "1234"))
        # 添加提取码变化监听，自动保存
        self.custom_extract_code_var.trace_add("write", self.on_extract_code_changed)
        ttk.Entry(extract_code_frame, textvariable=self.custom_extract_code_var).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        # 添加复制提取码按钮
        ttk.Button(extract_code_frame, text="复制提取码", command=lambda: self.copy_to_clipboard(self.custom_extract_code_var.get())).pack(side=tk.RIGHT, padx=5)
        ...
        ttk.Entry(password_frame, textvariable=self.current_password_var, state="readonly", show="").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        # 添加复制解压密码按钮
        ttk.Button(password_frame, text="复制解压密码", command=lambda: self.copy_to_clipboard(self.current_password_var.get())).pack(side=tk.RIGHT, padx=5)
        
        # 上传按钮
        self.upload_btn = ttk.Button(frame_baidu, text="上传到百度网盘", command=self.start_upload)
        ...
        ttk.Button(link_frame, text="复制", command=lambda: self.copy_to_clipboard(self.share_link_var.get())).pack(side=tk.RIGHT, padx=5)
        ...
        self.extract_code_entry = ttk.Entry(code_frame, textvariable=self.extract_code_var)
        self.extract_code_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        ttk.Button(code_frame, text="复制", command=lambda: self.copy_to_clipboard(self.extract_code_var.get())).pack(side=tk.RIGHT, padx=5)
    ...
    def create_settings_tab(self):
        ...
        ttk.Entry(frame_temp, textvariable=self.temp_path_var, state="readonly").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=10, pady=10)
        ttk.Button(frame_temp, text="浏览", command=self.browse_temp_path).pack(side=tk.RIGHT, padx=10, pady=10)
        ...
        self.promotion_tree = ttk.Treeview(promotion_list_frame, columns=columns, show="headings")
        ...
        # 添加滚动条
        scrollbar = ttk.Scrollbar(promotion_list_frame, orient=tk.VERTICAL, command=self.promotion_tree.yview)
        ...
        ttk.Button(button_frame, text="添加", command=self.add_promotion_file).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="修改", command=self.edit_promotion_file).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="删除", command=self.delete_promotion_file).pack(side=tk.LEFT, padx=5)
        
        # 宣传文件说明
        ttk.Label(frame_promotion, text="宣传文件可以是文件或文件夹，压缩时会自动处理").pack(anchor=tk.W, padx=10, pady=5)
        ttk.Label(frame_promotion, text="使用宣传文件时，解压密码将自动设置为对应的值").pack(anchor=tk.W, padx=10, pady=5)
        ttk.Label(frame_promotion, text="压缩完成后，宣传文件将自动从压缩包中删除").pack(anchor=tk.W, padx=10, pady=5)
        ...
        # 上传后自动删除文件选项
        self.auto_delete_after_upload_var = tk.BooleanVar(value=self.settings.get("auto_delete_after_upload", False))
        # 添加变化监听，自动保存
        self.auto_delete_after_upload_var.trace_add("write", self.on_settings_changed)
        auto_delete_check = ttk.Checkbutton(frame_upload, text="上传完成后自动删除本地压缩文件", variable=self.auto_delete_after_upload_var)
        auto_delete_check.pack(side=tk.LEFT, padx=5)
        
        # 保存设置按钮
        ttk.Button(self.tab_settings, text="保存设置", command=self.apply_settings).pack(padx=10, pady=20)
    ...
    def create_database_tab(self):
        ...
        ttk.Button(button_frame, text="刷新记录", command=self.refresh_compress_records).pack(side=tk.RIGHT, padx=5)
    ...
    def load_promotion_files(self):
        ...
        for i, file in enumerate(self.settings["promotion_files"]):
            self.promotion_tree.insert("", tk.END, values=(i+1, file["name"], file["path"], file["password"]))
    ...
    def create_promotion_dialog(self, edit_mode=False, item_id=None):
        ...
        ttk.Button(path_frame, text="浏览", command=browse_path).pack(side=tk.RIGHT, padx=5)
        ...
        ttk.Entry(pwd_frame, textvariable=pwd_var).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
...
备注：使用的是ai模型提取的文本可能会有误差，不过意思是差不多能听懂。介意误买。】"""
    ...
    def start_compress(self):
        ...
        messagebox.showinfo("提示", "压缩已开始，请稍后")
    ...
    def compress_folder(self):
        ...
        for dirpath, dirnames, filenames in os.walk(self.selected_folder):
            for filename in filenames:
                original_files.append(os.path.relpath(os.path.join(dirpath, filename), self.selected_folder))
            ...
            if os.path.isdir(promotion_path):
                ...
                for dirpath, dirnames, filenames in os.walk(promotion_path):
                    for filename in filenames:
                        ...
                        # 计算相对路径
                        relative_path = os.path.relpath(promotion_file_path, promotion_path)
                        # 目标路径
    ...
    def auto_upload(self, file_path, compress_id=None):
        ...
        try:
            ...
            self.share_link_var.set(share_info["link"] + f"?pwd={share_info['password']}")
    ...
    def manual_clean_promotion_files(self):
        """手动清理宣传文件"""
        if not hasattr(self, 'last_promotion_files_to_remove') or not self.last_promotion_files_to_remove:
    ...
    def start_upload(self):
        ...
        # 开始上传线程
        threading.Thread(target=self.upload_to_baidu, args=(upload_dir, extract_code), daemon=True).start()
